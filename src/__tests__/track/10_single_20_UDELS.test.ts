...
test('10_single_20_UDELS', () => {
  const testFiles = path.join(__dirname, '../../../test');
  const input = fs.readFileSync(path.join(testFiles,'/in/basic/10_single_20_UDELS.in'))
  const output = fs.readFileSync(path.join(testFiles,'/out/basic/10_single_20_UDELS.out'))
