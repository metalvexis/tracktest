...
test('10_single_20_UDE', () => {
  const testFiles = path.join(__dirname, '../../../test');
  const input = fs.readFileSync(path.join(testFiles,'/in/basic/10_single_20_UDE.in'))
  const output = fs.readFileSync(path.join(testFiles,'/out/basic/10_single_20_UDE.out'))
