...
test('10_single_20_LS', () => {
  const testFiles = path.join(__dirname, '../../../test');
  const input = fs.readFileSync(path.join(testFiles,'/in/basic/10_single_20_LS.in'))
  const output = fs.readFileSync(path.join(testFiles,'/out/basic/10_single_20_LS.out'))
