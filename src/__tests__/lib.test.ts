...
import { calcAllFee, calcTransferFee, calcStorageFee, calcUptimeFee, calcAllocUptime, calcAutoShutdownDate, parseDateToString, usageLimitToUptimeMinutes } from "../lib";
...
describe("LIBRARY", () => {
  ...
  test("Calc fees within free tier", () => {    
    ...
    const {
      ...
      transferFee,
      uptimeFee
    } = calcAllFee(initState, transfer, storage, uptime);
  ...
  test("Calc fees exceeding storage free tier", () => {    
    ...
    const expectFee = (excess / FEE_RATES.STORAGE.unit) * FEE_RATES.STORAGE.cost;
  ...
  test("Calc fees exceeding transfer free tier", () => {    
    ...
    const excess = 1 * SIZE_UNITS.GB;
    const expectFee = (excess / FEE_RATES.TRANSFER.unit) * FEE_RATES.TRANSFER.cost;
